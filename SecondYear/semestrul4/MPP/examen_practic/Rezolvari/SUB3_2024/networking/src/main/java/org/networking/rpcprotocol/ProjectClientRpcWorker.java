...
public class ProjectClientRpcWorker implements Runnable, ProjectObserver {
    ...
    private Response handleRequest(Request request) {
        ...
        try {
            switch (request.getType()) {
                ...
                case ADD_GAME:{
                    logger.info("Processing ADD_GAME request");
                    Game game = (Game) request.getData();
                    Game savedGame = server.addGame(game);
                    response = new Response.Builder().type(ResponseType.ADD_GAME).data(savedGame).build();
                    break;
                }
                case GET_ALL_GAMES:{
                    logger.info("Processing GET_ALL_GAMES request");
                    List<Game> allGames = server.getAllGames();
                    response = new Response.Builder().type(ResponseType.GET_ALL_GAMES).data(allGames).build();
                    break;
                }
                case GET_ALL_CONFIGURATIONS:{
                    logger.info("Processing GET_ALL_CONFIGURATIONS request");
                    List<Configuration> allGames = server.getAllConfigurations();
                    response = new Response.Builder().type(ResponseType.GET_ALL_CONFIGURATIONS).data(allGames).build();
                    break;
                }
                case FIND_PLAYER_BY_ALIAS:{
                    logger.info("Processing FIND_PLAYER_BY_ALIAS request");
                    String username = (String) request.getData();
                    User user = server.findPlayerByAlias(username);
                    response = new Response.Builder().type(ResponseType.FIND_PLAYER_BY_ALIAS).data(user).build();
                    break;
                }
                default:
    ...
    public void gameAdded(Game addedGame) {
        Response resp = new Response.Builder().type(ResponseType.UPDATE_GAME).data(addedGame).build();
        try {
            sendResponse(resp);
        } catch (IOException e) {
            e.printStackTrace();

        ...
        try {
            switch (request.getType()) {
                ...
                case GET_ALL_CONFIGURATIONS:{
                    logger.info("Processing GET_ALL_CONFIGURATIONS_VARSTA request");
                    List<Configuration> organizators = (List<Configuration>) server.getAllConfigurations();
                    response = new Response.Builder().type(ResponseType.GET_ALL_CONFIGURATIONS).data(organizators).build();
                    break;
                }
                case GET_ALL_WORDS:{
                    logger.info("Processing GET_ALL_WORDS_VARSTA request");
                    List<Word> organizators = (List<Word>) server.getAllWords();
                    response = new Response.Builder().type(ResponseType.GET_ALL_WORDS).data(organizators).build();
                    break;
                }
                case FIND_PLAYER_BY_ALIAS:{
                    logger.info("Processing FIND_PLAYER_BY_ALIAS request");
                ...
                case ADD_CONFIGURATION:{
                    logger.info("Processing ADD_CONFIGURATION_VARSTA request");
                    Configuration configuration = (Configuration) request.getData();
                    Configuration configuration1 = (Configuration) server.addConfiguration(configuration);
                    response = new Response.Builder().type(ResponseType.ADD_CONFIGURATION).data(configuration1).build();
                    break;
                }
                case ADD_CONFIGURATION_WORD:{
                    logger.info("Processing ADD_CONFIGURATION_WORD request");
                    ConfigurationWord configurationWord = (ConfigurationWord) request.getData();
                    ConfigurationWord configurationWord1 = (ConfigurationWord) server.addConfigurationWord(configurationWord);
                    response = new Response.Builder().type(ResponseType.ADD_CONFIGURATION_WORD).data(configurationWord1).build();
                    break;
                }
                default:

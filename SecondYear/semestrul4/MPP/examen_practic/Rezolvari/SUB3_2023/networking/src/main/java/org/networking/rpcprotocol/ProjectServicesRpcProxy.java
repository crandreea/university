...
public class ProjectServicesRpcProxy implements ProjectServices {
        ...
        Thread readerThread = new Thread(() -> {
            ...
            while (!finished) {
                try {
                    ...
                    if (response.getType() == ResponseType.UPDATE_GAMES) {
    ...
    @Override
    public Position addPosition(Position position) throws SQLException, ProjectException {
        Request req = new Request.Builder().type(RequestType.ADD_POSITION).data(position).build();
        Response response = sendRequestAndWait(req);
        if (response.getType() == ResponseType.ERROR) {
            String errorMessage = (response.getData() != null) ? response.getData().toString() : "Unknown login error";
            throw new ProjectException(errorMessage);
        ...
        return (Position) response.getData();
    }

    @Override
    public Configuration addConfiguration(Configuration configuration) throws SQLException, ProjectException {
        Request req = new Request.Builder().type(RequestType.ADD_CONFIGURATION).data(configuration).build();
        Response response = sendRequestAndWait(req);
        if (response.getType() == ResponseType.ERROR) {
            String errorMessage = (response.getData() != null) ? response.getData().toString() : "Unknown login error";
            throw new ProjectException(errorMessage);
    ...
    @Override
    public ConfigurationWord addConfigurationWord(ConfigurationWord configurationWord) throws SQLException, ProjectException {
        Request req = new Request.Builder().type(RequestType.ADD_CONFIGURATION_WORD).data(configurationWord).build();
        Response response = sendRequestAndWait(req);
        if (response.getType() == ResponseType.ERROR) {
            String errorMessage = (response.getData() != null) ? response.getData().toString() : "Unknown login error";
            throw new ProjectException(errorMessage);
        ...
        return (ConfigurationWord) response.getData();
    }
    ...
    @Override
    public List<Game> getAllGames() throws SQLException, ProjectException {
    ...
    @Override
    public List<Configuration> getAllConfigurations() throws SQLException, ProjectException {
    ...
    @Override
    public List<Word> getAllWords() throws SQLException, ProjectException {
        Request req = new Request.Builder().type(RequestType.GET_ALL_WORDS).build();
        Response response = sendRequestAndWait(req);
        if (response.getType() == ResponseType.ERROR) {
            String errorMessage = (response.getData() != null) ? response.getData().toString() : "Unknown login error";
            throw new ProjectException(errorMessage);
        ...
        return (List<Word>) response.getData();
    }

    @Override
    public User findPlayerByAlias(String alias) throws ProjectException {
        Request req = new Request.Builder().type(RequestType.FIND_PLAYER_BY_ALIAS).data(alias).build();
        Response response = sendRequestAndWait(req);
        if (response.getType() == ResponseType.ERROR) {
            String errorMessage = (response.getData() != null) ? response.getData().toString() : "Unknown login error";
            throw new ProjectException(errorMessage);

...
public class ProjectServicesRpcProxy implements ProjectServices {
    ...
    @Override
    public List<Position> getAllPositions() throws SQLException, ProjectException {
        Request req = new Request.Builder().type(RequestType.GET_ALL_POSITIONS).build();
        Response response = sendRequestAndWait(req);
        if (response.getType() == ResponseType.ERROR) {
            String errorMessage = (response.getData() != null) ? response.getData().toString() : "Unknown login error";
            throw new ProjectException(errorMessage);
        ...
        return (List<Position>) response.getData();
    }

    @Override
    public List<Position> getAllPositionsByGame(Game game) throws ProjectException {
        Request req = new Request.Builder().type(RequestType.GET_ALL_POSITIONS_BY_GAME).build();
        Response response = sendRequestAndWait(req);
        if (response.getType() == ResponseType.ERROR) {
            String errorMessage = (response.getData() != null) ? response.getData().toString() : "Unknown login error";
            throw new ProjectException(errorMessage);
    ...
    @Override
    public List<Position> findTraps(Game game) throws ProjectException {
        Request req = new Request.Builder().type(RequestType.GET_ALL_TRAPS).data(game).build();
        Response response = sendRequestAndWait(req);
        if (response.getType() == ResponseType.ERROR) {
            String errorMessage = (response.getData() != null) ? response.getData().toString() : "Unknown login error";
            throw new ProjectException(errorMessage);

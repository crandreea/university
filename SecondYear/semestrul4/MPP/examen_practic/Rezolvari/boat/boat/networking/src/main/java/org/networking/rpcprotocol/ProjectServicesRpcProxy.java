...
public class ProjectServicesRpcProxy implements ProjectServices {
        ...
        Thread readerThread = new Thread(() -> {
            ...
            while (!finished) {
                try {
                    ...
                    if (response.getType() == ResponseType.UPDATE_CLASAMENT) {
    ...
    private void handleUpdateResponse(Response response) {
        try {
            ...
            } else if (response.getType() == ResponseType.GAME_FINISHED) {
                logger.info("Received update notification");
                if (client != null) {
    ...
    public Game createGame(User user) throws ProjectException {
        Request req = new Request.Builder().type(RequestType.CREATE_GAME).data(user).build();
        Response response = sendRequestAndWait(req);
        if (response.getType() == ResponseType.ERROR) {
            String errorMessage = (response.getData() != null) ? response.getData().toString() : "Unknown login error";
            throw new ProjectException(errorMessage);
    ...
    public ShotResultDTO makeShot(Integer gameId, int x, int y) throws ProjectException {
        ...
        data[1] = x;
        data[2] = y;
        Request req = new Request.Builder().type(RequestType.MAKE_SHOT).data(data).build();
        Response response = sendRequestAndWait(req);
        if (response.getType() == ResponseType.ERROR) {
            String errorMessage = (response.getData() != null) ? response.getData().toString() : "Unknown login error";
            throw new ProjectException(errorMessage);
        }
        return (ShotResultDTO) response.getData();
    }
    ...
    public List<GamePosition> getGamePositions(Integer gameId) throws ProjectException {
        Request req = new Request.Builder().type(RequestType.GET_GAME_POSITIONS).data(gameId).build();
        Response response = sendRequestAndWait(req);
        if (response.getType() == ResponseType.ERROR) {
            String errorMessage = (response.getData() != null) ? response.getData().toString() : "Unknown login error";
            throw new ProjectException(errorMessage);
        }
        return (List<GamePosition>) response.getData();
    }
    ...
    public List<GamePosition> getBoatGamePositions(Integer gameId) throws ProjectException {
        Request req = new Request.Builder().type(RequestType.GET_BOAT_GAME_POSITIONS).data(gameId).build();
        Response response = sendRequestAndWait(req);
        if (response.getType() == ResponseType.ERROR) {
            String errorMessage = (response.getData() != null) ? response.getData().toString() : "Unknown login error";
            throw new ProjectException(errorMessage);
    ...
    public Game getGameById(Integer gameId) throws ProjectException {
        Request req = new Request.Builder().type(RequestType.GET_GAME_BY_ID).data(gameId).build();
        Response response = sendRequestAndWait(req);
        if (response.getType() == ResponseType.ERROR) {
            String errorMessage = (response.getData() != null) ? response.getData().toString() : "Unknown login error";
            throw new ProjectException(errorMessage);
    ...
    public List<Game> getFinishedGames() throws ProjectException {
        Request req = new Request.Builder().type(RequestType.GET_FINISHED_GAMES).build();
        Response response = sendRequestAndWait(req);
        if (response.getType() == ResponseType.ERROR) {
            String errorMessage = (response.getData() != null) ? response.getData().toString() : "Unknown login error";
            throw new ProjectException(errorMessage);

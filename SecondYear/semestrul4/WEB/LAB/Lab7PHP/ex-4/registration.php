...
if ($stmt->execute()) {
    try {
        ...
        $phpmailer->setFrom('no-reply@situl-tau.com', 'Laborator WEB');
        $phpmailer->addAddress($email);

        $phpmailer->isHTML(true);
        $phpmailer->Subject = 'Confirmare înregistrare cont';
        $link = "http://localhost:8000/confirmare.php?token=" . $token;
        $phpmailer->Body = "
            <h3>Bine ai venit!</h3>
            <p>Te rugam sa confirmi inregistrarea contului tau accesand urmatorul link:</p>
            <a href='$link'>$link</a>
            <br><br>
            <small>Daca nu ai cerut inregistrarea, ignora acest mesaj.</small>
        ";

        if ($phpmailer->send()) {
            echo "Se asteapta confirmarea emailului...";
        } else {
